...
def extract_user_and_text(message: Message, args: List[str]) -> (Optional[int], Optional[str]):
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found"):
            message.reply_text("I have never seen such a person here ... "
                                "Send me something like this tail or head (even if it's just a message) ..."
                                "Then we need to start work quickly ...")
        else:
